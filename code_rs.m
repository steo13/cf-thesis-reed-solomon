function GF_demo(p,m)
	...
	%Definizione dei parametri del campo di Galois e valutazione dei
	%parametri qualora non si passasse nulla alla funzione
    %caratteristica del campo GF(q) (potenza di un numero primo: p^m)
    if nargin < 2
        p = 2;
        m = 5;
    end
	...
	%Caratteristica del campo GF(q)
	q = p^m;
    
	GFx = GFx_init(GF);
	t = 2;

	%Definizione del polinomio generatore di un codice BCH
	...
	%Definizione del polinomio generatore di un codice Reed-Solomon
	g_RS = RS_poly(GFx,t);
    ...
    fprintf('\nDataword (dec): ');
    for i=1:size(dxdec,1)
        fprintf('%d ', dxdec(i, 1:size(dxdec,2)));
    end
    
    fprintf('\nDataword (poly): ');
    for i=1:size(dxdec,2)
        dx=poly2str(num2poly(GF, dxdec(1,i)));
        fprintf('%s\t\t', dx);
    end
    ...
    for i=1:size(dxdec,2)
        pxdec=mod(p^(2*t)*dxdec(1,i), g_RSdec);
        cxdec=(p^(2*t)*dxdec(1,i))+pxdec;
        fprintf('%s\t\t', poly2str(num2poly(GF,cxdec)));
    end
    fprintf('\n');
    
	subplot(2,1,1)
	G = GFx_dft(GFx,poly2xpoly(GFx,g_BCH));
	stem(-1:GFx.q-2,log2(G+2))
	set(gca,'XLim',[-1,GFx.q-2])
	ylabel('DFT')
...
%Inizializzazione delle variabili che descrivono il campo finito GF(p^m),
%il parametro p deve essere un numero primo, altrimenti la costruzione
%della tabella di divisione fallisce (e GF(p) non è un campo).
%Il parametro prim_poly definisce (nella notazione numerica convenzionale
%di num2poly/poly2num) il polinomio primitivo da utilizzare per la
%costruzione del campo GF(p^m) (in particolare per ottenere la
%moltiplicazione invertibile); se non viene passato un valore allora viene
%usato il primo (sempre rispetto a num2poly) polinomio primitivo di grado
%pari ad m

function GF = GF_init(p,m,prim_poly_num)

	%Definizione del numero primo
	GF.p = p;

	%Definizione del numero di componenti dei vettori del campo.
	GF.m = m;
    
	%Se considerati come polinomi, il grado massimo di essi è m - 1 ed
	%il polinomio avente tale grado massimo è il polinomio primitivo che 
    %serve a definire la moltiplicazione invertibile tra elementi aventi
    %grado superiore ad m

	%Definizione del umero di elementi del campo
	q = p^m;
	GF.q = q;

	%Nella rappresentazione polinomiale del campo occorre sapere
	%effettuare le operazioni aritmetiche tra gli elementi di GF(p) e le
	%operazioni polinomiali tra polinomi a coefficienti in GF(p).
	%Le operazioni aritmetiche +, - e * si effettuano in modulo p, mentre
	%per la divisione occorre una ricerca. Per efficienza, tabelliamo i
	%valori degli inversi degli elementi non nulli di GF(p)
    
    %Crezione di una matrice di zeri di dimensione 1*p
	inv = zeros(1,p);
	for i = 0:p-1
		for j = 0:p-1
			if mod(i*j,p) == 1
	...
	%Calcolo del polinomio primitivo qualora esso non venisse passato come
	%parametro
	if nargin < 3
		if false
		elseif q == 9
            ...
            %Ricerca del primo polinomio primitivo
